...
func GinBodyLogMiddleware(c *gin.Context) {
	...
	if err != nil && err.Error() != "EOF" {
		functions.ShowLog("GinBodyLogMiddleware", err.Error())
	}
	...
	allDataIO := map[string]interface{}{
		...
		"created_at":    functions.CurrentTime().In(loc).Format("2006-01-02T15:04:05"),
	}
	...
	if existAll, _ := functions.InArray(c.FullPath(), allLogs); existAll {
